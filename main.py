...
def parse_args():
    ...
    parser.add_argument(
        "--headless", action="store_true", help="Mode sans fenêtre (tests automatisés)"
    )
    parser.add_argument(
        "--noise",
        type=float,
...
def main():
    ...
    if args.agent:
        ...
        mode_label = (
            "joueur réel Supabase" if args.mode == "player" else "profil synthétique"
        )
        ...
        if not agent:
            try:
                ...
                summary = generate_session_summary(
                    features, all_sessions, player_sessions
                )
                ...
                url = (
                    f"http://127.0.0.1:8050/?player={args.player_name}"
