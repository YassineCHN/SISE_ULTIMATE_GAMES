...
class BaseGame(ABC):
    ...
    def run(self) -> dict:
        ...
        if not self.headless:
            ...
            mode_label = (
                f"AGENT — {self.agent.profile_name}"
                if self.agent else self.game_id
            )
    ...
    def _show_game_over_screen(self):
        ...
        surf4 = font_small.render(
            "Fermeture automatique...", True, (150, 150, 150)
        )
...
#         self.controller = agent if agent else Controller()
#         #self.controller = Controller()
#         self.recorder = SessionRecorder(player_name, game_id=self.game_id)
#         self.running = False
#         self.clock = pygame.time.Clock()
...
#             #controller_state = self.controller.get_state()
#             if self.agent:
#                 controller_state = self.agent.get_state()   # ← agent joue
#             else:
#                 controller_state = self.controller.get_state()  # ← humain joue
