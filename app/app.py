...
def page_game(theme, df_real=None):
    ...
    html.Div(id="live-buttons-display"),
        ...
        # ── Score moyen par jeu ──
        make_card([
            html.Div("Score moyen par jeu", style={"color": t["subtext"], "fontSize": "11px",
                                                    "textTransform": "uppercase", "letterSpacing": "2px", "marginBottom": "12px"}),
            dcc.Graph(figure=make_score_bar(theme, df_real), config={"displayModeBar": False}),
        ], theme),
    ])
...
def page_profils(theme, df_real=None):
    ...
    return html.Div([
        html.Div([
            html.Div([
                html.Div("🧬 Profils comportementaux", style={"color": t["accent1"], "fontSize": "22px",
                                                               "fontWeight": "700", "fontFamily": t["font"], "marginBottom": "4px"}),
                html.Div("Clustering — projection des joueurs par style de jeu",
                         style={"color": t["subtext"], "fontSize": "13px"}),
            ]),
        ...
        html.Div([
            make_card([
                html.Div(axe_label, style={"color": t["subtext"], "fontSize": "11px",
                                           "textTransform": "uppercase", "letterSpacing": "2px", "marginBottom": "12px"}),
                dcc.Graph(id="umap-graph", figure=make_umap_fig(theme, df_real),
                          config={"displayModeBar": False}),
            ], theme, {"flex": "2"}),

            make_card([
                html.Div("Profil sélectionné", style={"color": t["subtext"], "fontSize": "11px",
                                                       "textTransform": "uppercase", "letterSpacing": "2px", "marginBottom": "12px"}),
                dcc.Dropdown(
                    id="profile-selector",
                    ...
                    style={"background": t["card"], "color": t["text"],
                           "border": f"1px solid {t['border']}", "borderRadius": "6px", "marginBottom": "12px"},
                    className="custom-dropdown",
                ),
                dcc.Graph(id="radar-graph", figure=make_radar_fig("Précis", theme),
                          config={"displayModeBar": False}),
                html.Div([
                    html.Div([
        ...
        html.Div([
            make_card([
                html.Div("Features moyennes par cluster", style={"color": t["subtext"], "fontSize": "11px",
                                                                   "textTransform": "uppercase", "letterSpacing": "2px", "marginBottom": "16px"}),
                html.Table([
                    html.Thead(html.Tr([
                        html.Th(col, style={"color": t["subtext"], "fontSize": "11px", "padding": "8px 12px",
                                            "textAlign": "left", "borderBottom": f"1px solid {t['border']}"})
                        for col in ["Profil","Réactivité","Agressivité","Fluidité","Précision","Risque","Consistance","Nb joueurs"]
                    ])),
                    html.Tbody([
                        html.Tr([
                            html.Td(v, style={"color": t["text"] if i > 0 else color, "padding": "8px 12px",
                                              "fontFamily": t["font_body"], "fontSize": "13px",
                                              "borderBottom": f"1px solid {t['border']}"})
                            for i, v in enumerate(row)
                        ])
                        for row, color in [
                            (["Agressif",  "0.40","0.95","0.50","0.60","0.90","0.50","—"], "#FF4C6A"),
                            (["Prudent",   "0.80","0.20","0.75","0.85","0.20","0.90","—"], "#00E5FF"),
                            (["Précis",    "0.90","0.50","0.90","0.95","0.50","0.85","—"], "#69FF47"),
                            (["Chaotique", "0.60","0.80","0.30","0.40","0.85","0.30","—"], "#FFB800"),
                        ]
                    ]),
    ...
    return html.Div([
        html.Div([
            html.Div([
                html.Div("🤖 Agent Imitateur", style={"color": t["accent1"], "fontSize": "22px",
                                                       "fontWeight": "700", "fontFamily": t["font"], "marginBottom": "4px"}),
                html.Div("L'IA rejoue les patterns comportementaux d'un profil appris",
                         style={"color": t["subtext"], "fontSize": "13px"}),
            ]),
            data_badge(is_real, theme),
        ], style={"display": "flex", "justifyContent": "space-between",
        ...
        html.Div([
            make_card([
                ...
                html.Div("Joueur cible", style={"color": t["subtext"], "fontSize": "11px", "marginBottom": "6px"}),
                dcc.Dropdown(
            ...
            html.Div([
                ...
                make_card([
                    html.Div("Score de similarité comportementale", style={"color": t["subtext"], "fontSize": "11px",
                                                                             "textTransform": "uppercase", "letterSpacing": "2px", "marginBottom": "16px"}),
                    html.Div([
                        html.Div("92.4%", style={"color": t["accent1"], "fontSize": "48px",
                                                  "fontFamily": t["font"], "fontWeight": "700", "textAlign": "center"}),
                        html.Div("de similarité avec le profil Agressif",
                                 style={"color": t["subtext"], "fontSize": "13px", "textAlign": "center", "marginTop": "4px"}),
                        html.Div(html.Div(style={
...
def update_page(g, p, c, a):
    ...
    return {"nav-game":"game","nav-profils":"profils",
            "nav-classifier":"classifier","nav-agent":"agent"}.get(btn, "game")
