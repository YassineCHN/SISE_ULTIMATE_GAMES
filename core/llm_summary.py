...
def _build_prompt(features, all_sessions, player_sessions):
    ...
    if all_sessions:
        ...
        rank_global = next(
            (i + 1 for i, s in enumerate(scores_sorted) if s <= features.score), n_total
        )
    ...
    if len(player_sessions) > 1:
        ...
        rank_personal = next(
            (i + 1 for i, s in enumerate(p_scores) if s <= features.score), n_player
        )
        personal_best = max(s.get("score", 0) for s in player_sessions)
        personal_avg = round(
            sum(s.get("score", 0) for s in player_sessions) / n_player, 0
        )
        progression = round(features.score - personal_avg, 0)
    else:
        rank_personal, personal_best, personal_avg, progression = (
            1,
            features.score,
            features.score,
            0,
    ...
    global_avg = (
        round(sum(s.get("score", 0) for s in all_sessions) / max(n_total, 1), 0)
        if all_sessions
        else 0
    )
    ...
    btn_interp = (
        "très peu de boutons (joueur calme/stratège)"
        if features.btn_press_rate < 1
        else (
            "rythme d'appuis modéré (joueur équilibré)"
            if features.btn_press_rate < 2.5
            else (
                "fréquence élevée (joueur frénétique/agressif)"
                if features.btn_press_rate < 4
                else "fréquence extrême (joueur ultra-réactif)"
            )
        )
    )
    joystick_interp = (
        "mouvements très précis et maîtrisés"
        if features.lx_std < 0.15
        else (
            "mouvements fluides et contrôlés"
            if features.lx_std < 0.30
            else (
                "mouvements amples et dynamiques"
                if features.lx_std < 0.45
                else "mouvements chaotiques/imprévisibles"
            )
        )
    )
    regularity_interp = (
        "rythme très régulier et méthodique"
        if features.input_regularity < 0.1
        else (
            "rythme assez constant"
            if features.input_regularity < 0.25
            else (
                "jeu saccadé avec des accélérations"
                if features.input_regularity < 0.5
                else "timing très irrégulier/impulsif"
            )
        )
    )
    vs_avg = (
        "au-dessus de la moyenne"
        if features.score > global_avg
        else "en-dessous de la moyenne"
    )
    prog_txt = (
        f"+{progression:.0f} au-dessus de ta moyenne"
        if progression >= 0
        else f"{progression:.0f} par rapport à ta moyenne"
    )
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if response.status_code == 429:
                ...
                print(
                    f"⏳ Mistral rate limit — attente {wait}s (tentative {attempt}/{MAX_RETRIES})"
                )
                time.sleep(wait)
                continue
            ...
            print(
                f"✅ Résumé IA généré pour {features.player_name} @ {features.game_id}"
            )
            return result
...
def generate_and_save_async(features, all_sessions=None, player_sessions=None):
    ...
    t.start()
    print(
        "🧠 Résumé IA en cours (visible dans le dashboard onglet Résumés sous ~30s)..."
    )
    return t
...
def _mock_summary(features):
    game_label = GAME_LABELS.get(features.game_id, features.game_id)
    btn_label = (
        "très calme et stratège"
        if features.btn_press_rate < 1
        else "équilibré" if features.btn_press_rate < 2.5 else "agressif et frénétique"
    )
    joy_label = (
        "précis et contrôlés"
        if features.lx_std < 0.25
        else "amples et dynamiques" if features.lx_std < 0.45 else "imprévisibles"
    )
    return {
        ...
        "profil_joueur": (
            "Stratège Posé"
            if features.btn_press_rate < 1 and features.lx_std < 0.25
            else (
                "Tireur de Précision"
                if features.lx_std < 0.2
                else (
                    "Assaillant Frénétique"
                    if features.btn_press_rate > 3
                    else "Joueur Équilibré"
                )
            )
        ),
        "points_forts": [
